...
### Next.js API route vs. separate API service

The app uses a Next.js API route (`app/api/rates/route.ts`) instead of a separate backend service because the server-side work is intentionally narrow: validate query parameters, load the local store, run `lib/query.ts`, and return JSON. Keeping that endpoint inside the Next.js app avoids duplicating project setup, deployment configuration, environment handling, CORS rules, and type definitions for a single read-only query surface.

This also keeps the frontend and query contract close together. The UI calls one local route, the route imports the same TypeScript types and query helpers as the rest of the app, and local development stays as simple as `npm run dev` after ingesting the data. For a take-home project focused on parsing and searching MRF data, that simplicity matters more than introducing an additional service boundary.

A separate API would make more sense if the backend needed independent scaling, authentication shared by multiple clients, background jobs, database connection pooling, or a public API consumed outside this Next.js app. Those are reasonable future directions, but they would add operational weight before this implementation needs it.
